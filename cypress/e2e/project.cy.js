describe("PGK project Architectural Style Image Collector", () => {
  before(() => {
    cy.task("ensureReportDir");
  });

  it("Collect line art and grayscale render images from all project portfolio pages", () => {
    cy.task("fetchAllProjectLinks", null, { timeout: 120000 }).then((portfolioLinks) => {
      ...
      cy.task("scanAllPagesForGrayImages", portfolioLinks, {
        ...
        cy.task("saveReport", {
          ...
          filename: "project-gray-image-report.json",
        });
      });
    });
  });
